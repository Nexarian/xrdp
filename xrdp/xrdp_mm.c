...
process_dynamic_monitor_description(struct xrdp_wm *wm, struct display_size_description *description)
{
    ...
    {
        LOG(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: Not allowing resize. Suppress output is active.");
        return 0;
    }
    if (description == NULL)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: description is null.");
        return 0;
    }
    if (description->session_width <= 0 || description->session_height <= 0)
    {
        LOG(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: Not allowing resize due to invalid dimensions (w: %d x h: %d)", description->session_width, description->session_height);
        return 0;
    }
    ...
    if (error != 0)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: libxrdp_reset failed %d", error);
        return error;
    }
    /* reset cache */
    ...
    if (error != 0)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: xrdp_cache_reset failed %d", error);
        return error;
    }
    /* load some stuff */
    ...
    if (error != 0)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: xrdp_wm_load_static_colors_plus failed %d", error);
        return error;
    }
    ...
    if (error != 0)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: xrdp_wm_load_static_pointers failed %d", error);
        return error;
    }
    ...
    if (error != 0)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: xrdp_bitmap_resize failed %d", error);
        return error;
    }
    /* redraw */
    ...
    if (error != 0)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: xrdp_bitmap_invalidate failed %d", error);
        return error;
    }
    ...
    if (module != 0)
    {
        ...
        if (error != 0)
        {
            LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: mod_server_version_message failed %d", error);
            return error;
        }
        error = module->mod_server_monitor_resize(module, description->session_width, description->session_height);
        if (error != 0)
        {
            LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: mod_server_monitor_resize failed %d", error);
            return error;
        }
        error = module->mod_server_monitor_full_invalidate(module, description->session_width, description->session_height);
        if (error != 0)
        {
            LOG_DEVEL(LOG_LEVEL_ERROR, "\tprocess_dynamic_monitor_description: mod_server_monitor_full_invalidate failed %d", error);
            return error;
        }
    }
    ...
    g_memcpy(wm->client_info->minfo, description->minfo, sizeof(struct monitor_info) * CLIENT_MONITOR_DATA_MAXIMUM_MONITORS);
    g_memcpy(wm->client_info->minfo_wm, description->minfo_wm, sizeof(struct monitor_info) * CLIENT_MONITOR_DATA_MAXIMUM_MONITORS);
    return 0;
}
