...
xorgxrdp_helper_inf_init(void)
{
    ...
    {
        LOG(LOG_LEVEL_ERROR, "glx_ver too old %d", glx_ver);
        return 1;
    }
                                 ...
                                 "GLX_EXT_texture_from_pixmap"))
    {
        ...
        LOG(LOG_LEVEL_ERROR, "GLX_EXT_texture_from_pixmap not present [%s]", ext_str);
        return 1;
    }
                                  ...
                                  void *cdata, int *cdata_bytes)
{
    ...
    if (vertices == NULL)
    {
        LOG(LOG_LEVEL_ERROR, "error get_vertices failed num_crects %d", num_crects);
        return ENCODER_ERROR;
    }
